...
Regex flags are a thorny corner case with regexes, in that they're both highly platform specific, and (in their usual usage) don't compose properly because of their global nature (and regex composition is the entire point of `wreck`).  As a result `wreck` makes the opinionated design choice to only natively support grouped embedded flags (e.g. `(?i:[abc]+)`), as these provide scoping of the flag(s) to a well-defined portion of a regex (i.e. the contents of the group).  This avoids virtually all of the problems of global flags, with the downside that the JVM has 2 flags that can't be embedded, and JavaScript has 5.
